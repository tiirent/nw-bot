...
class Vision:
    ...
    def __init__(self, needle_img_path, method=cv.TM_CCOEFF_NORMED):
        ...
        self.needle_img_flipped = cv.flip(self.needle_img, 1)
        ...
        self.name = needle_img_path[6:].split(".")[0]

    def find(self, haystack_img, threshold=0.5, max_results=10, flip=True):
        # run the OpenCV algorithm
        result = cv.matchTemplate(haystack_img, self.needle_img, self.method)
        if flip:
            result = result + cv.matchTemplate(haystack_img, self.needle_img_flipped, self.method)
    ...
    def draw_rectangles(self, haystack_img, rectangles, addText=False):
        ...
        for (x, y, w, h) in rectangles:
            ...
            if addText:
                cv.putText(haystack_img, self.name, (x, y-10), cv.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
